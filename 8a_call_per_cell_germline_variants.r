...
files.res = foreach(barcode.file = barcode.files) %do% {
    ...
    while(counter < length(sample.barcodes)){
        ...
        barcode.vcf.updated = paste0(barcode.vcf,".samplenameupdated.vcf")
        barcode.vcf.gz = paste0(barcode.vcf.updated,".gz")
        ...
        cmds.list[["update_vcf"]] = c(cmds.list[["update_vcf"]] , paste0("perl ./8b_update_vcf_sample_name.pl ",barcode.vcf,tail))
        
        cmds.list[["bgzip"]] = c(cmds.list[["bgzip"]] , paste0("bgzip -f ",barcode.vcf.updated,tail))
